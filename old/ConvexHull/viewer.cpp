...
void Viewer::convexHull(MAP& m_map, VertexAttribute<VEC3, MAP> m_positions, std::vector<VEC3> pts_of_hull)
{
	assert(pts_of_hull.size() > 4);

	CellMarker<MAP, FACE> mf_to_supress(m_map);
	CellMarker<MAP, FACE> mf_convex_hull(m_map);

	//create a first tetrahedron
	Dart d = Algo::Surface::Modelisation::createTetrahedron<PFP>(m_map,false);
...
//	mf_convex_hull.mark(d);
//	mf_convex_hull.mark(m_map.phi2(d));
//	mf_convex_hull.mark(m_map.phi2(m_map.phi1(d)));
//	mf_convex_hull.mark(m_map.phi2(m_map.phi_1(d)));
	...
	for(unsigned int i = 4; i < pts_of_hull.size() ; i++)
	{
		...
		TraversorF<MAP> tf(m_map);
		for(Dart f = tf.begin() ; f != tf.end() ; f = tf.next())
		{
//			if(mf_convex_hull.isMarked(f))
//			{
...
//				}
		...
		if(outside)
		{
			...
			//merge marked faces
			// foreach face of the convex hull
			TraversorE<MAP> te(m_map);
			for(Dart e = te.begin() ; e != te.end() ; e = te.next())
			{
				if(mf_to_supress.isMarked(e))
					std::cout << "ismarked = " << e << std::endl;

				if(mf_to_supress.isMarked(e) && mf_to_supress.isMarked(m_map.phi2(e)))
				{
					toDel.push_back(e);
				}
				else if(mf_to_supress.isMarked(e)&& !mf_to_supress.isMarked(m_map.phi2(e)))
					dboundary = e;
			}

			mf_to_supress.unmarkAll();

			for(unsigned int i = 0 ; i < toDel.size() ; i++)
			{
				//std::cout << "deg=" << m_map.edgeDegree(toDel[i]) << "\t d=" << toDel[i] << std::endl;
				m_map.mergeFaces(toDel[i]);
			}
			...
			// marked the faces around the vertex as in the convex hull
//			Traversor3VF<MAP> tvf(m_map, dcenter);
//			for(Dart d = tvf.begin() ; d != tvf.end() ; d =tvf.next())
//			{
//				mf_convex_hull.mark(d);
//			}
		}

		std::cout << "one vertex added" << std::endl;

	}

}

void Viewer::initMesh()
{
	...
	lstPointsOfHull.push_back(VEC3(0,0,0));
	lstPointsOfHull.push_back(VEC3(0,0,1));
	lstPointsOfHull.push_back(VEC3(0,1,0));
	lstPointsOfHull.push_back(VEC3(0,1,1));
	lstPointsOfHull.push_back(VEC3(1,0,0));
	lstPointsOfHull.push_back(VEC3(1,0,1));
	lstPointsOfHull.push_back(VEC3(1,1,0));
	lstPointsOfHull.push_back(VEC3(1,1,1));
	lstPointsOfHull.push_back(VEC3(2,1,4));

	convexHull(myMap, position, lstPointsOfHull);

	
	md = new Utils::Drawer();
	md->newList(GL_COMPILE);
	md->begin(GL_POINTS);
	md->pointSize(10000.0f);
	md->color3f(125.0,0.0,0.0);

	for(unsigned int i = 0 ; i < lstPointsOfHull.size() ; ++i)
	{
		md->vertex(lstPointsOfHull[i]);
	}
	md->end();
	md->endList();
